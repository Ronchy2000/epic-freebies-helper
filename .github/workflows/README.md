...
新的工作流还会额外上传 `epic-screenshots-<run_id>`。如果日志里只看到 `Timeout waiting for #email`、`Just a moment...`、`One more step` 这类提示，优先看截图 artifact。
...
5. 把上面这些 zip 直接拖进 issue 编辑框，或者点击附件按钮上传。
